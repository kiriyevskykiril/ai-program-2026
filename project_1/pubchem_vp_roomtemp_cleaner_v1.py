...
# -*- coding: utf-8 -*-
"""
pubchem_vp_roomtemp_cleaner_v1.py
...
Run:
    python pubchem_vp_roomtemp_cleaner_v1.py input.csv output_vp_roomtemp.csv

Test one CID:
    python pubchem_vp_roomtemp_cleaner_v1.py --test-cid 244
"""
...
TEMP_RE = re.compile(
    r"(?P<t>-?\d+(?:\.\d+)?)\s*°?\s*(?P<tunit>C|F|K|c|f|k|degrees?\s*C|degrees?\s*F|kelvin)\b",
    re.IGNORECASE,
)
...
def nearby_temperature_c(text: str, span: Tuple[int, int], window: int = 80) -> Tuple[Optional[float], str]:
    ...
    for m in TEMP_RE.finditer(nearby):
        ...
        temps.append(tc)
    if temps:
        # choose the closest temperature occurrence in the window; practically this is enough for PubChem strings
        return temps[0], "explicit_temperature_near_value"
...
def parse_vp_candidates(text: str, temp_min_c: float = 15.0, temp_max_c: float = 30.0, accept_no_temp: bool = False) -> List[Dict[str, Any]]:
    ...
    matches = list(VP_VALUE_UNIT_RE.finditer(raw)) + list(VP_UNIT_VALUE_RE.finditer(raw))
    ...
    for m in matches:
        ...
        temp_c, temp_reason = nearby_temperature_c(raw, m.span())
        ...
        candidates.append({
            ...
            "vp_raw_pubchem": raw,
            "vp_parse_score": candidate_score(raw, temp_c, temp_reason, temp_min_c, temp_max_c),
        })
    ...
    seen = set()
    for c in candidates:
        key = (c["vp_mmhg_pubchem"], c["vp_temperature_c"], c["vp_unit_original"])
        if key not in seen:
            seen.add(key)
            unique.append(c)
    return unique
...
def process_file(input_file: Path, output_file: Path, autosave_every: int, cache_dir: Optional[Path], sleep: float, no_resume: bool, temp_min_c: float, temp_max_c: float, accept_no_temp: bool) -> None:
        ...
        try:
            ...
            if best:
                ...
                if vp_texts:
                    ...
                    out["vp_raw_pubchem_clean"] = " | ".join(vp_texts[:5])
                else:
